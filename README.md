...
ローカルでビルドする際は、Dockerを利用する次のコマンドを実行します。

Build your application with the `pipenv run build --use-container` command with docker in local.
...
pipenvにて生成した `src/requirements.txt` ファイルを元に、SAM-CLIがデプロイパッケージを作成した `.aws-sam/build` ディレクトリに配置します。

The SAM-CLI installs dependencies defined in `src/requirements.txt` to be generated by pipenv, creates a deployment package, and saves it in the `.aws-sam/build` folder.

1つのLambda関数を実行するためには、 `events/event.json` ファイルを利用すると良いです。
...
```bash
pipenv run local-invoke
```

API自体をローカル実行する場合は、次のコマンドを実行してください。

The SAM-CLI can also emulate this application's API.
Use the `sam local start-api` to run the API locally on port 3000.

```bash
pipenv run local-start-api
